...
struct QrCodeScannerView: UIViewRepresentable {
    ...
    private let metadetaOutput = AVCaptureMetadataOutput()
    ...
    func setupCamera(_ uiView: CameraPreview) {
        if let backCamera = AVCaptureDevice.default(for: AVMediaType.video) {
            if let input = try? AVCaptureDeviceInput(device: backCamera) {
                ...
                if session.canAddOutput(metadetaOutput) {
                    session.addOutput(metadetaOutput)
                    
                    metadetaOutput.metadataObjectTypes = supportedBarcodeTypes
                    metadetaOutput.setMetadataObjectsDelegate(delegate, queue: DispatchQueue.main)
                }
